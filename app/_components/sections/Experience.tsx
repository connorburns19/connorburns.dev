                ...
                <div>
                  {job.companyUrl ? (
                    <a
                      ...
                      className="font-semibold text-text hover:text-text-dim transition-colors"
                    >
                      {job.company}
                    </a>
                  ) : (
                    <span className="font-semibold text-text">{job.company}</span>
                  )}
                  <span className="text-sub-dim mx-2">·</span>
                  <span className="text-text-dim">{job.title}</span>
                </div>
                <span className="text-sm text-text-dim whitespace-nowrap shrink-0">
                  {job.period}
                </span>
              </div>
              <ul className="space-y-1.5">
                {job.bullets.map((bullet) => (
                  <li key={bullet} className="flex gap-2 text-sm text-text-dim leading-relaxed">
                    <span className="text-sub-faint mt-0.5 shrink-0">–</span>
                    {bullet}
                  </li>
                ))}
              </ul>
